...
def scheme_read(src):# src是buffer的实例，(参考buffer.py)，注意，无法使用BUffer[1]来索引。
    ...
    >>> scheme_read(Buffer(tokenize_lines(['(car \'(1 2))'])))
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # BEGIN PROBLEM 1
            "*** YOUR CODE HERE ***"
            if src.current() == '(' or 'nil':
                #/.首先脑海里想到 ['2 (3 4))']的例子--> Pair(2, Pair(Pair(3, Pair(4, nil))), nil)，特别注意出现了连续两次Pair.并且最后还有nil，所以，先是return scheme_read, 作为Pair的first元素，再继续递归tail作为第二个元素。
                return Pair(scheme_read(src),read_tail(src))
            elif src.current() in quotes:# !!!忽视处理的地方,要在tail也处理quote的情况。第一个pair.first表示quote+左+右括号结束，第二个pair.second表示除去quote(),scr后面的继续递归。
                return Pair(Pair(quotes[src.pop_first()], Pair(scheme_read(src),nil)), read_tail(src))
            else:
