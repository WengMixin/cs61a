...
def scheme_eval(expr, env, tail=False): # Optional third argument is ignored
    ...
    if scheme_listp(expr):
        ...
        if scheme_symbolp(first) and first in SPECIAL_FORMS:
            ...
            if tail:
                return Thunk(operator, operands)
            else:
...
def scheme_apply(procedure, args, env,tail=False):
    ...
    if isinstance(procedure, BuiltinProcedure):
        ...
        return eval_all(procedure.body, new_env,tail=tail)
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            ...
            while i != nil and j!=nil:
                child_frame.define(i.first,j.first)
                i,j = i.rest,j.rest
            return child_frame
        # END PROBLEM 10
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        #使用LambdaProcedure的self.env制作新框架，不影响父框架env的变量。arg的框架在全局变量，而我们的formals(假设x，y)在lambda子框架，我们需要把x,y绑定到全局变量中，所以make_child需要在子框架(self.env)里面"继承"而不是全局框架(env)。
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #因为expression.rest是一个list()包含着一个expr,所以为了得到expr必须取first值。
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        clause = expressions.first #clause就是去掉括号的全部表达式
        validate_form(clause, 1)
        if clause.first == 'else': #如果第一个元素就是else，则直接让test等于true，(test的作用是确定条件语句是否等于false), 直接跳到最里面的if，进行后续判断。
            test = True 
            if expressions.rest != nil: #expressions.rest指else语句后面还有语句，直接报错
                raise SchemeError('else must be last')
        else: #如果不是else，则后续要进行t/f判断。判断前先将结果赋值给test
            test = scheme_eval(clause.first, env)
        if is_true_primitive(test): #如果if成立，则test的值不等于false，也就是test有可能等于true，也有可能等于所有数字。
            # BEGIN PROBLEM 13
            "*** YOUR CODE HERE ***"
            if clause.rest is nil: #且没有后续语句，则直接返回test的结果。t或者数字
                return test
            else: #否则，返回后续语句的计算结果。
                return eval_all(clause.rest,env)      
            # END PROBLEM 13
        expressions = expressions.rest #如果一轮下来，test的结果等于false，则继续执行后续的cond语句。
    return None #如果所有的connd语句都都等于False.则返回None。
...
def make_let_frame(bindings, env):
    ...
    # END PROBLEM 14
    # (let ((x (+ y 2)) (y (+ x 2))) (cons x (cons y nil)))
...
class MuProcedure(Procedure):
    ...
    "*** YOUR CODE HERE ***"
    def make_call_frame(self, args, env):
        #新调用框架的父级是过程被评估的环境(env),而不是全局框架self.env(MuProcedure也没有这个框架anyway)，也就是动态作用域。在代码的不同部分调用相同的过程可能会导致不同的结果
...
class Thunk(object):
    ...
    def evaluate(self):
        """Evaluate the thunk's expression in its environment."""
...
def complete_apply(procedure, args, env,tail=False):
    ...
    validate_procedure(procedure)
    val = scheme_apply(procedure, args, env,tail=tail)
    if isinstance(val, Thunk):
        return scheme_eval(val.expr, val.env,tail=tail)
    else:
        return val

def scheme_iter(expr, env):
    """Iteratively evaluate Scheme expression EXPR in environment ENV,
    using tail calls when possible."""
    while True:
        if scheme_symbolp(expr):
            return env.lookup(expr)
        elif self_evaluating(expr):
            ...
            if scheme_symbolp(first) and first in SPECIAL_FORMS:
                ...
                if operator.is_compound():
                    expr = operator.body
                    env = operator.make_call_frame(rest, env)
                else:
                    return operator.apply(rest, env)
        else:
            raise SchemeError("Cannot evaluate {0}".format(repl_str(expr)))
...
def optimize_tail_calls(prior_eval_function):
    ...
    def optimized_eval(expr, env, tail=False):
        ...
        while True:
            if scheme_symbolp(expr):
                ...
                return nil
            elif scheme_symbolp(expr.first) and expr.first in SPECIAL_FORMS:
                result = SPECIAL_FORMS[expr.first](expr.rest, env)
                if tail and not isinstance(result, Thunk):
                    return result
                expr = result
            else:
    ...
    def trampoline(thunk):
        """Evaluate a Thunk until a non-Thunk value is returned."""
